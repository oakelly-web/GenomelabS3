...
```{r}
list.files("..")              # what’s one level up?
...
# Build site × taxonomy table, dropping rows with missing site info
site_taxa <- NEON_MAGs %>%
...
Perfect — now we’re moving from **alpha diversity (within-site)** to **beta diversity (between-site)**. Beta diversity compares how similar or different communities are across sites.

### 🌱 Bray–Curtis Dissimilarity

-   **Definition:** A measure of compositional dissimilarity between two sites based on counts or abundances.

-   **Formula:** BCij=∑∣xik−xjk∣∑(xik+xjk) where xik and xjk are abundances of species k in sites i and j.

-   **Range:** 0 (identical composition) → 1 (completely different).
...
-   **Range:** 0 (identical phylogenetic composition) → 1 (completely distinct).
...
# mat = site × taxa abundance matrix (already built earlier)
...
MDS is robust for ecological data and uses dissimilarity (e.g., Bray–Curtis).

```{r}
library(vegan)
library(ggplot2)

# Run NMDS on your site × taxa matrix
